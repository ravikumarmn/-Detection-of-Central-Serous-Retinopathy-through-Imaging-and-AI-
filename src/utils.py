...
def load_split_data(file_name_str = "train_val_test",model_type = None):
    ...
    train_data = datasets["train"]['train_images'].float() # The training data tensor
    train_labels = datasets["train"]['train_labels'] # The training labels tensor

    val_data = datasets["validation"]['val_images'].float() # The validation data tensor
    val_labels = datasets["validation"]['val_labels'] # The validation labels tensor
    ...
    if model_type == "cnn":
        ...
        print(f"Validation data size : {datasets['metadata']['test_size']}")
...
def get_result(model_obj,name, save_file):
    ...
    if name =="cnn":
        ...
        y_train = X_test
        y_test = torch.cat([y for _,y in X_test]).numpy()
    else:
...
def train_fn(dataloader, model, optimizer, criterion, device="cpu"):
    ...
    for inputs, labels in dataloader:
        ...
        predicted = torch.tensor([int(p >= 0.5) for p in pred_prob.view(-1)])
        # _, predicted = torch.max(outputs.data, 1)
        # loss = criterion(outputs.squeeze(-1), labels)
        loss = criterion(pred_prob.view(-1), labels.float())
        loss.backward()
        optimizer.step()
        total += labels.size(0)
