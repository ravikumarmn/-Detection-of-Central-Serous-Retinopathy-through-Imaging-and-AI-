...
if data_option == "FUNDUS":
  ...
  model_state_dict_file = "checkpoints/data_fundus_model_cnn_learning_rate_0.001_batch_size_32.pt"
  ...
  model_state_dict_file = "checkpoints/data_macular_model_cnn_learning_rate_0.0001_batch_size_8.pt"
  file = st.file_uploader(
    "Upload the image to be classified U0001F447",
    type=["jpg", "png","jpeg"],
    accept_multiple_files=False,
...
def upload_predict(batch_t, model):
    with torch.no_grad():
        ...
        predicted = torch.tensor([int(p >= 0.5) for p in prob])
